...
MemoryBuffer::MemoryBuffer(char *pointer, size_t size) : _size(size) {
  this->setg(pointer, pointer, pointer + this->_size);
}

std::streambuf::pos_type MemoryBuffer::seekoff(off_type pos, std::ios_base::seekdir dir,
  ...
  if (dir == std::ios_base::beg) {
    ...
    const off_type currentPosition = (off_type)this->gptr() - (off_type)this->eback();
